...
begin
  ...
  {$ENDIF}
  {$ENDIF}
  if nQHD and (gSysParam.FDepartment='监装') then
  begin
    cxRadio1.Properties.ReadOnly := True;
    cxRadio1.Enabled := False;
  end;
...
begin
  ...
  with ADOQuery1 do
  try
    if gSysParam.FDepartment='监装' then
    begin
      //不显示已出厂车辆
      nStr := 'Select * from %s Where E_Date>=%s-1 and E_Result Is Null ' +
              'and E_Departmen=''%s'' and not exists(select 1 from %s where l_id=LEFT(e_id,len(e_id)-1) and l_status=''O'') Order By R_ID ASC';
      nStr := Format(nStr, [sTable_ManualEvent, sField_SQLServer_Now,
              gSysParam.FDepartment,sTable_Bill,sFlag_TruckOut]);      
    end
    else begin
      nStr := 'Select * from %s Where E_Date>=%s-1 and E_Result Is Null ' +
              'and E_Departmen=''%s'' Order By R_ID ASC';
      nStr := Format(nStr, [sTable_ManualEvent, sField_SQLServer_Now,
              gSysParam.FDepartment]);
    end;
    //一天之内本部门未处理消息
