...
begin
  ...
  nStr := 'C_Name=Select C_Name From %s ';
...
procedure TfFormHYData.EditCustomPropertiesChange(Sender: TObject);
var
  nPos:Integer;
begin
  nPos:=Pos('.',EditCustom.Text);
  if nPos>0 then
    EditName.Text:=Copy(EditCustom.Text,nPos+1,Length(EditCustom.Text)-nPos)
  else
    EditName.Text:=EditCustom.Text;
end;
