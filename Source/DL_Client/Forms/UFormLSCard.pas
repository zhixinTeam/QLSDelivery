...
type
  TfFormLSCard = class(TfFormNormal)
...
class function TfFormLSCard.CreateForm(const nPopedom: string;
...
begin
  ...
  with TfFormLSCard.Create(Application) do
  try
...
class function TfFormLSCard.FormID: integer;
begin
  Result := cFI_FormLSCard;
end;

procedure TfFormLSCard.ActionComPort(const nStop: Boolean);
var nInt: Integer;
    nIni: TIniFile;
begin
  if nStop then
...
procedure TfFormLSCard.LoadFormData(const nID: string);
var nStr: string;
begin
  if nID <> '' then
  begin
...
begin
  ...
  nStr := MakeSQLByStr([SF('I_Card', EditCard.Text),
          ...
          ], sTable_InOutFatory, nStr, FRecord = '');
  FDM.ExecuteSQL(nStr);

  nStr := Format('C_Card=''%s''', [EditCard.Text]);
  nStr := MakeSQLByStr([SF('C_Status', sFlag_CardUsed),
          SF('C_Used', sFlag_Other),
...
procedure TfFormLSCard.FormClose(Sender: TObject;
...
procedure TfFormLSCard.ComPort1RxChar(Sender: TObject; Count: Integer);
var nStr: string;
    nIdx,nLen: Integer;
    nCard:string;
begin
...
initialization
  gControlManager.RegCtrl(TfFormLSCard, TfFormLSCard.FormID);
