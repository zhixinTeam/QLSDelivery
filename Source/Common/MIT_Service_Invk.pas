...
type
  ...
  TSrvAxmsg_Invoker = class(TROInvoker)
  private
  protected
  public
    constructor Create; override;
...
constructor TSrvAxmsg_Invoker.Create;
begin
  inherited Create;
  FAbstract := False;
end;

procedure TSrvAxmsg_Invoker.Invoke_DL2WRZSINFO(const __Instance:IInterface; const __Message:IROMessage; const __Transport:IROTransport; out __oResponseOptions:TROResponseOptions);
{ function DL2WRZSINFO(const BusinessType: Utf8String; const XMLPrimaryKey: Utf8String): Boolean; }
var
  BusinessType: Utf8String;
  XMLPrimaryKey: Utf8String;
  lResult: Boolean;
begin
  try
    __Message.Read('BusinessType', TypeInfo(Utf8String), BusinessType, []);
    __Message.Read('XMLPrimaryKey', TypeInfo(Utf8String), XMLPrimaryKey, []);

    lResult := (__Instance as ISrvAxmsg).DL2WRZSINFO(BusinessType, XMLPrimaryKey);

    __Message.InitializeResponseMessage(__Transport, 'MIT_Service', 'SrvAxmsg', 'DL2WRZSINFOResponse');
    __Message.Write('Result', TypeInfo(Boolean), lResult, []);
    __Message.Finalize;
    __Message.UnsetAttributes(__Transport);
