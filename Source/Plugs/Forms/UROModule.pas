...
uses
  ...
  SrvAxmsg_Impl, MIT_Service_Invk;
...
procedure Create_SrvAxmsg(out anInstance : IUnknown);
begin
  anInstance := TSrvAxmsg.Create;
end;
...
begin
  ...
  begin
    ...
    FSrvAxmsg := TROPooledClassFactory.Create('SrvAxmsg',
                Create_SrvAxmsg, TSrvAxmsg_Invoker,
                FPoolSizeBusiness, TRoPoolBehavior(FPoolBehaviorBusiness));
  end;
end;
...
begin
  ...
  if Assigned(FSrvAxmsg) then
  begin
    UnRegisterClassFactory(FSrvAxmsg);
    FSrvAxmsg := nil;
  end;
end;
...
begin
  try
    ...
    begin
      ...
      if (FSrvConnection = nil) or (FSrvBusiness = nil) or (FSrvAxmsg = nil) then
        RegClassFactories;
