...
type
  ...
  public
    ...
    procedure SetReaderCardExt(const nReader,nExtCard: string);
    //手动读卡
    function GetCardLastDone(const nCard,nReader: string): Int64;
    procedure SetCardLastDone(const nCard,nReader: string);
    function GetReaderLastOn(const nCard: string; var nExtRID:string): string;
    //磁卡活动
...
var nReader: string;
begin
  Result := GetPoundCard(nPound, nReader);
end;
...
//Date: 2016-08-22
//Parm: 读头号;磁卡号
//Desc: 设置nReader上的扩展卡信息，用于特殊业务比对
procedure THardwareHelper.SetReaderCardExt(const nReader, nExtCard: string);
var nIdx: Integer;
begin
  FSyncLock.Enter;
  try
...
function THardwareHelper.GetReaderLastOn(const nCard: string; var nExtRID:string): string;
...
begin
  ...
  try
    ...
    begin
      ...
      nTP := NodeByName('ExtReader');
      if Assigned(nTP) then
           FExtReader := nTP.ValueAsString
      else FExtReader := '';
