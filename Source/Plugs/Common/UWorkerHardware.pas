...
begin
  ...
  begin
    ...
    {$IFDEF DEBUG}
    WriteLog(FIn.FData+' ::: '+FOut.FData);
    {$ENDIF}
    
    //gHardwareHelper.SetPoundCardExt(FIn.FData, FOut.FData);
    gHardwareHelper.SetReaderCardExt(nReader, FOut.FData);
    //将远距离卡号对应的近距离卡号绑定
  end;
end;
...
function THardwareCommander.OpenPoundDoor(var nData: string): Boolean;//道闸抬杆
var
  nFID,nExtFID:string;
begin
  Result:=False;
  try
    ...
    Result:=True;
    WriteLog(nData);
    if (FIn.FExtParam<>'0') and (nExtFID='') then
      FOut.FData:= '无需道闸抬杆'
    else
      FOut.FData:= '道闸抬杆OK';
  except
